...
역할을 `시스템관리자/인사담당자/팀장/일반` 4개에서 `사용자/관리자` 2개로 단순화한다
(사용자=인사팀, 전 기능 / 관리자=+로그·계정·설정). 이미 0001~0011 이 적용된 배포 DB 에는
Studio SQL 편집기에서 **`supabase/migrations/hotfix_0012_two_roles.sql`** 전체를 그대로 복붙해
실행한다(재실행해도 안전). 신규로 처음부터 적용하는 경우엔 `0012_two_roles.sql` 을 순서대로
적용하면 된다(또는 최신 `APPLY_ALL.sql` 기준으로 다시 합쳐 넣는다).
